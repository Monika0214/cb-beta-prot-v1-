...
const AVATAR_OPTIONS = [
  ...
  'https://api.dicebear.com/7.x/pixel-art/svg?seed=Simba',
...
export const EditProfile: React.FC<EditProfileProps> = ({ user, onBack, onUpdateProfile }) => {
  ...
  const handleNameSave = () => {
    if (!isUsernameEligible || tempName === user.name || !tempName.trim()) return;
    onUpdateProfile({ name: tempName, lastUsernameChange: Date.now() });
    onBack();
  };

  return (
    <div className="flex flex-col h-full bg-black animate-in slide-in-from-bottom duration-400">
      {/* HEADER */}
      <div className="px-6 py-10 border-b border-zinc-900 flex items-center bg-zinc-950/95 backdrop-blur-md sticky top-0 z-50">
        <button 
          onClick={onBack}
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-10 pb-32">
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-center">
             <h3 className="heading-font text-4xl font-black italic text-white uppercase tracking-tighter">{user.name}</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="flex overflow-x-auto no-scrollbar gap-4 py-2 flex-nowrap items-center h-24">
            {AVATAR_OPTIONS.map((avatar) => {
              ...
              return (
                <div 
                  ...
                  className={`flex-shrink-0 w-20 h-20 rounded-full border-2 p-1 cursor-pointer transition-all ${
                ...
                >
                  ...
                  {isSelected && (
                    <div className="absolute -top-1 -right-1 bg-red-600 text-white rounded-full p-1 border-2 border-zinc-950">
                      <Check size={10} />
                    </div>
                  )}
                </div>
        ...
        <div className="space-y-4 pt-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
               ...
               <p className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em]">USERNAME</p>
            </div>
            {!isUsernameEligible && (
          ...
          <div className="space-y-4">
            ...
            <div className="px-2">
              {isUsernameEligible ? (
                ...
                <p className="text-[10px] font-black text-red-500/50 uppercase tracking-widest leading-relaxed">
                  LOCKED: USERNAME CAN BE CHANGED AGAIN IN <span className="text-red-600">{daysToWait} DAYS</span>.
                </p>
              )}
            </div>
          </div>
        ...
        {/* SAVE ACTION */}
        {isUsernameEligible && tempName !== user.name && tempName.trim().length > 0 && (
          <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-full max-w-lg px-6 z-50">
            <button 
              onClick={handleNameSave}
              className="w-full bg-red-600 hover:bg-red-500 text-white py-5 rounded-2xl heading-font text-3xl font-black italic uppercase tracking-widest shadow-[0_20px_50px_rgba(220,38,38,0.3)] transition-all active:scale-95 border-b-4 border-red-800"
            >
              SAVE CHANGES
            </button>
          </div>
